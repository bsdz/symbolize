...
from .expression import Symbol, ApplicationExpression, alias_render_latex, alias_render_unicode
...
class IntegralExpression(ApplicationExpression):
    ...
    def render_unicode(self, renderer):  # @UnusedVariable
        integrand, limit_min, limit_max = self.children
        dummy_var = integrand.children[0]
        return "%s%s (%s=%s..%s)" % tuple([e.render_unicode(renderer) for e in (self.base, integrand.base, dummy_var, limit_min, limit_max)])
    ...
    def render_unicode(self, renderer):
        if not hasattr(renderer, '_inclusion_exclusion_groups'):
            ...
            def _render_unicode_postfix(renderer):
                if renderer._inclusion_exclusion_groups:
                    return "[%s]" % ", ".join(["%s %s %s" % (", ".join([e.render_unicode(renderer) for e in mem_expr_set]), 
                                      oper_collect[0].render_unicode(renderer), oper_collect[1].render_unicode(renderer))
                     for oper_collect, mem_expr_set in renderer._inclusion_exclusion_groups.items()])
...
class LogicQuantificationExpression(ApplicationExpression):
    ...
    @alias_render_unicode
    def render_unicode(self, renderer):  # @UnusedVariable
        return "%s%s.%s" % (self.base.render_unicode(renderer),
