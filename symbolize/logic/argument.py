...
class Argument:
    ...
    def repr_latex(self):
        ...
        if self._discharges:
            ...
            %s\\
            \end{matrix}""" % (
                r" \quad ".join([p.repr_latex() for p in self._discharges]),
                top_tex,
            )

        return r"\frac{%s}{%s}%s" % (
            top_tex,
            self._conclusion.repr_latex(),
    ...
    def repr_html(self):
        ...
        </table>""" % """</td><td style="vertical-align:bottom">""".join(
            [
                p.repr_html() if hasattr(p, "repr_html") else p.repr_unicode()
