...
class Proposition:
    ...
    def __init__(self, baserepr, arity=None):
        self.proof_expr = (
            baserepr if isinstance(baserepr, Expression) else Symbol(baserepr, arity)
        )

    def repr_latex(self):
        return "%s : %s" % (
            self.proof_expr.repr_latex(),
            self.__class__.proposition_expr.repr_latex(),
        )
...
def get_proposition_class(expr):
    ...
    if type_string in globals():
        ...
        return type(type_string, (Proposition,), {"proposition_expr": expr})
