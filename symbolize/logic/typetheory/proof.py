...
class PropositionException(Exception):
    pass
...
class ProofExpression(Expression, metaclass=ProofExpressionMetaClass):
    ...
    def repr_typestring(self):
        ...
        if self.proposition_type is None:
            ...
            return "%s : %s" % (
                TypeStringRenderer().render(self),
                TypeStringRenderer().render(self.proposition_type),
            )
    ...
    def repr_latex(self):
        ...
        if self.proposition_type is None:
            ...
            return "%s : %s" % (
                ...
                LatexRenderer().render(self.proposition_type),
            )
    ...
    def repr_unicode(self):
        ...
        if self.proposition_type is None:
            ...
            return "%s : %s" % (
                UnicodeRenderer().render(self),
                UnicodeRenderer().render(self.proposition_type),
            )
    ...
    def apply(self, *expressions, **kwargs):
        ...
        new_prop_type = self.apply_proposition_type(
            expressions, **apply_proposition_type_kwargs
        )
    ...
    def abstract(self, *expressions):
        ...
        if self.proposition_type.contains_free(expressions[0]):
            if (
                self.proposition_type.arity != forall.arity.lhs.args[1]
            ):  # @UndefinedVariable
                warn("RHS doesn't match forall arity. forcing.")
                self.proposition_type.arity = forall.arity.lhs.args[
                    1
                ]  # @UndefinedVariable
            new_prop_type = forall(expressions[0], self.proposition_type)
        else:
            new_prop_type = implies(
                expressions[0].proposition_type, self.proposition_type
            )
        return super().abstract(
            *expressions, abstraction_kwargs={"proposition_type": new_prop_type}
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(args[1].proposition_type, PropositionSubstitutionExpression):
            self.proposition_type = exists(
                args[1].proposition_type.old, args[1].proposition_type
            )
        else:
...
class ProofAbstractionExpression(
    ...
    def apply_proposition_type(self, expressions, **kwargs):
        ...
        if self.proposition_type.base == forall:
            # [ST] p90
            return self.proposition_type.children[1].substitute(
                self.proposition_type.children[0], expressions[0]
            )
