...
class TestDeductionRules(unittest.TestCase):
    ...
    def test_conjunction_elimination(self):
        ...
        self.assertEqual(
            s11.proof_expr,
            fst(ExpressionCombination(p.proof_expr, q.proof_expr)),
            "proof has correct expr",
        )
        self.assertEqual(
            s12.proof_expr,
            snd(ExpressionCombination(p.proof_expr, q.proof_expr)),
            "proof has correct expr",
        )
    ...
    def test_disjunction_elimination(self):
        ...
        A_implies_C = get_proposition_class(
            implies(A.proposition_expr, C.proposition_expr)
        )
        B_implies_C = get_proposition_class(
            implies(B.proposition_expr, C.proposition_expr)
        )
